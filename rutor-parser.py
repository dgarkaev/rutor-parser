...
CREATE UNIQUE INDEX  IF NOT EXISTS ifn ON abooks (
    fullname
);"""

con = sqlite3.connect('rutor.db')
con.executescript(sql_make)
...
while True:
    ...
    for row in rows:
        ...
        if nm.lower().endswith('mp3'):
            ...
            # sql_add = "INSERT OR IGNORE INTO abooks(fullname,sz,url,magnet) VALUES(?,?,?,?)"
            # sql_add = "INSERT OR REPLACE INTO abooks VALUES(?,?,?,?,?,?,?,?,?,?,?)"
            rz = con.execute("INSERT OR IGNORE INTO abooks(fullname,sz,url,magnet) VALUES(?,?,?,?)", 
                             (nm, sz, 'http://rutor.info'+aurl, magnet,))
            # update: если запись уже в БД - выходим (только при INSERT OR IGNORE)
            if update == True and rz.rowcount == 0:
